...
from models.options.options_pb2 import XGBoostGridSearchGestureLocalizationOptions, Datasets
from models.options.utils import load_options, make_description
from utils.logger import Logger

tf.logging.set_verbosity(tf.logging.INFO)
log = Logger('XGBoostSearch')
...
def main(options_filename):
    op = load_options(options_filename, XGBoostGridSearchGestureLocalizationOptions)
    ...
    xgboost = XGBClassifier(vervosity=2,
    ...
    params_filename = join(op.storage.logs, "xgboost_best_params.json")
    with open(params_filename, 'w') as f:
        json.dump(gs.best_params_, f, indent=2, sort_keys=True)

    results_filename = join(op.storage.logs, "xgboost_results.json")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        XGBoostGridSearchGestureLocalizationOptions protobuf message.""")
